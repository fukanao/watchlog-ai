...
Slackには危険度「高」「中」を検知・判定した時点で通知し、「小」「低」は毎朝9:00（日本時間）の1投稿にまとめます。
日次報告は前回以降に蓄積した判定件数・検知ログ・要約（最大20種類）を掲載し、対象がなければ投稿しません。
「高」「中」と同時に検知した「小」「低」もSlackの即時投稿から分離します。
メールとRaspberry Piなどのwebhookは、従来どおり「高」「中」と遮断ログの「小」を通知します。

危険度の意味:

- 高: いますぐ対応が必要。脆弱性を突かれた、または情報が漏洩した。
- 中: 攻撃検知。回数が多い、または攻撃らしいが情報漏洩は確認できない。同一IPから失敗した不正アクセスが10回以上連続した場合も中。
- 小: nginxの遮断ログに記録されたアクセス。アプリケーションには未到達。
- 低: スキャン程度。
- 無: 正常アクセス。
...
- `STATE_FILE`: 読み取り位置、日次報告の集約内容・送信日を保存するJSONファイル
- Ollama に接続できない場合、`SLACK_WEBHOOK_URL` が設定されていれば最大 1 時間に 1 回 Slack へ不達通知を送ります。

日次報告には常駐実行が必要です。通常の監視間隔が長くても、待機中は9:00に処理を再開します。
AI判定中に9:00を過ぎた場合は、判定処理が終了してから送信します。
サービス停止で送信時刻を過ぎた場合は再開時に送信し、送信済みの日は再投稿しません。
送信失敗時は集約内容を保持し、次の監視処理で再試行します。再起動しても未送信の内容を引き継ぎます。
`--once` も実行時に送信時刻を過ぎた未送信の報告があれば送信します。
